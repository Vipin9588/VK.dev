...
export default function InstagramCard({ dark }) {
  ...
  return (
    <div
      className={`${dark ? "border  border-gray-800" : "bg-[rgba(234,235,236,0.92)]"}  rounded-2xl shadow-sm overflow-hidden w-full h-full`}
    >
      <div className="flex items-center justify-between px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p
              className={`text-[10px] ${dark ? "text-white" : "text-gray-400"} leading-none`}
            >
              Location Here
            </p>
          </div>
        ...
        <BsThreeDots
          className={`${dark ? "text-white" : "text-gray-500"} text-base cursor-pointer`}
        />
      </div>
      ...
      <div className="px-3 pt-2 pb-3">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-3">
            ...
            >
              {liked ? (
                ...
                <FaRegHeart
                  className={`text-base ${dark ? "text-white" : "text-gray-700"}`}
                />
              )}
            </button>
            <FaRegComment
              className={`text-base ${dark ? "text-white" : "text-gray-700"} cursor-pointer`}
            />
            <LuSend
              className={`text-base ${dark ? "text-white" : "text-gray-700"} cursor-pointer`}
            />
          ...
          <button onClick={() => setSaved((s) => !s)}>
            {saved ? (
              <FaBookmark
                className={`text-base ${dark ? "text-white" : "text-gray-900"}`}
              />
            ) : (
              <FaRegBookmark
                className={`text-base ${dark ? "text-white" : "text-gray-700"} cursor-pointer`}
        ...
        <p
          className={`text-[11px] font-semibold ${dark ? "text-white" : "text-gray-900"} flex`}
        >
          {likes.toLocaleString()} likes
        </p>
        <p
          className={`flex text-[11px] ${dark ? "text-white" : "text-gray-800"} mt-0.5 leading-snug`}
        >
          <span className="flex font-semibold">youraccount</span> Enhance your
          Instagram with our UI Mockup.
        </p>
